...
{
	public partial class frmYZDepartYearData : Form
	{
		...
		private void LoadData()
		{
			...
			DataTable dttSQL1 = k3db.GetDataTable("SELECT FItemID,FName FROM dbo.t_Department WHERE FNumber LIKE '3007.%' ORDER BY FNumber", "A1");
			...
			cobFDepartID.DataSource = dttSQL1;
			...
			cobFDepartIDQ.DataSource = dttSQL1Q;
		...
		{
			string sSQL = string.Empty;
			sSQL += " SELECT D.*,e.FCode,  FName='['+p1.Fname+']-'+'['+p2.Fname+']-'+'['+p3.Fname+']',FDepartName=t.FName  FROM YZ_DepartYearData d  ";
			sSQL += " INNER JOIN YZ_projectList e  ON d.FProjectID=e.FID  ";
			sSQL += " INNER JOIN dbo.t_Department t ON t.FItemID=d.FDepID  ";
			sSQL += " left join YZ_project P1 on e.FPID1=P1.FID    ";
			sSQL += " left join YZ_project P2 on e.FPID2=P2.FID  ";
			sSQL += " left join YZ_project P3 on e.FPID3=P3.FID  ";
			...
			{
				edtFYear.Text = dttSQL.Rows[0]["FYear"].ToString().Trim()+"-01-01";
		...
		private void frmYZDepartYearData_Load(object sender, EventArgs e)
		{
		...
		private void btnSave_Click(object sender, EventArgs e)
		{
			...
			if (MasterState == "ADD")
			{
				sSQL = " SELECT count(*) from YZ_DepartYearData where FYear=@FYear and FDepID=@FDepID and FProjectID=@FProjectID  ";

				SqlParameter[] par = new SqlParameter[] 
			           {
			              new SqlParameter("@FYear",Convert.ToDateTime( edtFYear.Text.Trim()+"-01-01").Year),
			...
			{
				int FID = Convert.ToInt32(((DataRowView)bdsMaster.Current).Row["FID"]);
				sSQL = " SELECT count(*) from YZ_DepartYearData where FYear=@FYear and FDepID=@FDepID and FProjectID=@FProjectID and FID<>@FID    ";
			...
			if (MasterState == "ADD")
			{
				sSQL = " insert into YZ_DepartYearData(FYear,FDepID,FProjectID,FValue,FText) values (@FYear,@FDepID,@FProjectID,@FValue,@FText) ";
				SqlParameter[] par = new SqlParameter[] 
			          {
			               new SqlParameter("@FYear",Convert.ToDateTime( edtFYear.Text.Trim()+"-01-01").Year),
									  new SqlParameter("@FDepID", cobFDepartID.SelectedValue),
			...
			{
				sSQL = " update YZ_DepartYearData set FYear=@FYear,FDepID=@FDepID,FProjectID=@FProjectID,FValue=@FValue,FText=@FText  where FID=@FID";
				SqlParameter[] par = new SqlParameter[] 
			          {
		...
		private void button1_Click(object sender, EventArgs e) //查询
		{
			...
			{
				sSQLWH += " AND d.FYear='" + edtFYearQ.Text + "' ";
			}
		...
		private void btnDelete_Click(object sender, EventArgs e)
		{
			...
			if (MessageBox.Show("确定要删除当前数据？", "软件提示", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
			{
				int FID = Convert.ToInt32(((DataRowView)bdsMaster.Current).Row["FID"]);
				string sSQL = string.Empty;
				sSQL = "DELETE from YZ_DepartYearData where  FID=@FID  ";
