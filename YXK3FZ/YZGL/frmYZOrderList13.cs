...
{
	public partial class frmYZOrderList13 : Form
	{

		DataBase k3db = null;

		public frmYZOrderList13()
		{
			InitializeComponent();
		...
		private void frmYZOrderList13_Load(object sender, EventArgs e)
		{
			LoadPower();//加载权限
		...
		private void getDate(string sSQLWH) //加载数据
		{
			...
			dtr["FSG"] = dttSQL2.Compute("Sum(FSG)", "");
			dtr["FYF"] = dttSQL2.Compute("Sum(FYF)", "");
			dtr["FSC"] = dttSQL2.Compute("Sum(FSC)", "");
			dtr["FCF"] = dttSQL2.Compute("Sum(FCF)", "");
			dtr["FJL"] = dttSQL2.Compute("Sum(FJL)", "");
		...
		private bool getBillState(string sType)
		{
			if (bdsMaster.Current != null)
			{
				...
				sSQL += " FROM TAB13 where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
				DataTable dttSQL = k3db.GetDataTable(sSQL, "A");
		...
		private void btnSel_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				frmYZOrder13 frmYZOrder13 = new frmYZOrder13("SEL", ((DataRowView)bdsMaster.Current).Row["ID"].ToString());
				frmYZOrder13.Text = frmYZOrder13.Text;
				frmYZOrder13.Show();
		...
		{
			frmYZOrder13 frmYZOrder13 = new frmYZOrder13("ADD", "");
			frmYZOrder13.Text = frmYZOrder13.Text;
			frmYZOrder13.Show();
		}
		//修改
		private void tsbtnEdit_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				if (getBillState("修改"))
				{
					frmYZOrder13 frmYZOrder13 = new frmYZOrder13("EDIT", ((DataRowView)bdsMaster.Current).Row["ID"].ToString());
					frmYZOrder13.Text = frmYZOrder13.Text;
					frmYZOrder13.Show();
				}
		...
		private void btnDelete_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				...
				if (MessageBox.Show("确定要删除当前单据吗？", "系统提示", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
				{
					...
					{
						string sSQL = string.Empty;
						sSQL = " delete from  TAB13 where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
		...
		private void btnCheck_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				...
				{
					string sSQL = string.Empty;
					sSQL = " update TAB13 set FState=1,FCheckUser='" + PropertyClass.OperatorName + "',FCheckDate='" + k3db.GetCurrentDate() + "' where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
					if (k3db.ExecDataBySql(sSQL) > 0)
					{
		...
		private void btnHcheck_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				...
				{
					string sSQL = string.Empty;
					sSQL = " update TAB13 set FState=0,FCheckUser=NULL,FCheckDate=NULL where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
					if (k3db.ExecDataBySql(sSQL) > 0)
					{
