...
{
    public partial class frmYZOrder07 : Form
    {
			...
			/// <param name="sID">内码ID</param>
					public frmYZOrder07(string sMasterState, string sID)
          {
              InitializeComponent();
			...
			private void LoadDate(string sID)
			{
				...
				if (dttSQL.Rows.Count > 0)
				{
					...
					txtFCommSW.Text = dttSQL.Rows[0]["FCommSW"].ToString();
					...
					edtFDALL.Text = dttSQL.Rows[0]["FDALL"].ToString();
					edtFXFB.Text = dttSQL.Rows[0]["FXFB"].ToString();
					edtFDRS.Text = dttSQL.Rows[0]["FDRS"].ToString();
					edtFBMLD.Text = dttSQL.Rows[0]["FBMLD"].ToString();
					edtFAQK.Text = dttSQL.Rows[0]["FAQK"].ToString();
					txtFPAQK.Text = dttSQL.Rows[0]["FPAQK"].ToString();
					edtFDZP.Text = dttSQL.Rows[0]["FDZP"].ToString();
					txtFGT.Text = dttSQL.Rows[0]["FGT"].ToString();
          ...
          //把内码地磅值添加进来
					edtDB.Text = dttSQL.Rows[0]["FDBPD"].ToString();
				}
				else
				{
			...
			private void btnSave_Click(object sender, EventArgs e)
			{
				...
				{
					MessageBox.Show("油料加工量(度)不能为空");
					edtFAmountOfFinish.Focus();
					return;
				}
				//总表(1+2)-耗量(度)
				if (edtFDALL.Text == string.Empty)
				{
					MessageBox.Show("总表(1+2)-耗量(度)不能为空");
					edtFDALL.Focus();
					return;
				}
				...
				{
					MessageBox.Show("车间-耗量(度)不能为空");
					edtFCar.Focus();
					return;
				}
				if (edtFGL.Text == string.Empty)
				{
					MessageBox.Show("锅炉-耗量(度)不能为空");
					edtFGL.Focus();
					return;
				}
				...
				{
					MessageBox.Show("仓储筒仓-耗量(度)不能为空");
					edtFStoreTC.Focus();
					return;
				}
				if (edtFStoreYK.Text == string.Empty)
				{
					MessageBox.Show("仓储油库-耗量(度)不能为空");
					edtFStoreYK.Focus();
					return;
				}
				if (edtFStorePK.Text == string.Empty)
				{
					MessageBox.Show("仓储粕库-耗量(度)不能为空");
					edtFStorePK.Focus();
					return;
				}
				...
				{
					MessageBox.Show("工程机修-耗量(度)不能为空");
					edtFCommJX.Focus();
					return;
				}

				//if (txtFCommSW.Text == string.Empty)
				//{
				//  MessageBox.Show("工程水务-耗量(度)不能为空");
				//  txtFCommSW.Focus();
				//  return;
				//}
				...
				{
					MessageBox.Show("工程配电-耗量(度)不能为空");
					edtFCommPD.Focus();
					return;
				}

				if (edtFXFB.Text == string.Empty)
				{
					MessageBox.Show("消防泵-耗量(度)不能为空");
					edtFXFB.Focus();
					return;
				}
				if (edtFDRS.Text == string.Empty)
				{
					MessageBox.Show("豆软水-耗量(度)不能为空");
					edtFDRS.Focus();
					return;
				}
				if (edtFBMLD.Text == string.Empty)
				{
					MessageBox.Show("北门路灯-耗量(度)不能为空");
					edtFBMLD.Focus();
					return;
				}


				if (edtFAQK.Text == string.Empty)
				{
					MessageBox.Show("安全科-耗量(度)不能为空");
					edtFAQK.Focus();
					return;
				}
				...
				{
					MessageBox.Show("公共设施-耗量(度)不能为空");
					edtFCommSS.Focus();
					return;
				}

				if (edtFDZP.Text == string.Empty)
				{
					MessageBox.Show("豆制品-耗量(度)不能为空");
					edtFDZP.Focus();
					return;
				}
				//增加地磅编辑框
				if (edtDB.Text == string.Empty)
				{
					MessageBox.Show("地磅-耗量(度)不能为空");
					edtDB.Focus();
					return;
				}
				...
				double F68 = Convert.ToDouble(edtFCommJX.Text.Trim());
				double F74 = Convert.ToDouble(edtFXFB.Text.Trim());
				double F75 = Convert.ToDouble(edtFDRS.Text.Trim());
				double F76 = Convert.ToDouble(edtFBMLD.Text.Trim());
				//增加地磅编辑框
				double F79 = Convert.ToDouble(edtDB.Text.Trim());

				txtFCommSW.Text = (F74 - F75 - F76 - F68).ToString();
				...
				double F61 = Convert.ToDouble(txtFCommSW.Text);
				...
				double F77 = Convert.ToDouble(edtFAQK.Text);
				if (F42 == 0)
				{
					txtFPAQK.Text = "0";
				}
				...
				{
					txtFPAQK.Text = Math.Round((F77 * 1000 / F42), 2).ToString();
				}
				//逻辑增加扣减地磅的数值
				double A = Convert.ToDouble(edtFDALL.Text) - Convert.ToDouble(edtFCar.Text) - Convert.ToDouble(edtFStoreTC.Text) - Convert.ToDouble(edtFStoreYK.Text) - Convert.ToDouble(edtFStorePK.Text) - Convert.ToDouble(edtFCommPD.Text) - Convert.ToDouble(edtFXFB.Text) - Convert.ToDouble(edtFAQK.Text) - Convert.ToDouble(edtFCommSS.Text) - Convert.ToDouble(edtFDZP.Text) - Convert.ToDouble(edtDB.Text);

				txtFGT.Text = A.ToString();

				double F82 = Convert.ToDouble(txtFGT.Text);


				txtFWater.Text = Math.Round((F45 + F47 + F49 + F57 + F65 + F77 + F82), 2).ToString();
				...
				sSQL = " select * from TAB07 where FClassID='" + cobFClassID.SelectedValue + "' ";
				...
				if (MasterState == "ADD") //新增保存
				{
					...
					sSQL += " FDALL,FXFB,FDRS,FBMLD,FAQK,FPAQK,FDZP,FGT,";
					sSQL += " FRemark,FNewUser	,FNewDate	,FState,FDBPD	 ";
					sSQL += " )  ";
					sSQL += " VALUES  ";
					sSQL += " (@FClassID, @FBoatID ,@FDate,@FRunTimeByHour	,@FDownTimeByHour	,	@FAmountOfFinish	, ";
					sSQL += " @FWater,@FPWater,@FCar,@FPCar,@FGL,@FPGL,	 ";
					...
					SqlParameter[] par = new SqlParameter[] 
                 {
                     ...
                     new SqlParameter("@FBoatID",cobFBoatID.SelectedValue),
										 ...
										 new SqlParameter("@FCommSW",txtFCommSW.Text.Trim()),
							      ...
							      new SqlParameter("@FDALL",edtFDALL.Text.Trim()),	
							 	    new SqlParameter("@FXFB",edtFXFB.Text.Trim()),	
								 	  new SqlParameter("@FDRS",edtFDRS.Text.Trim()),	
									 	new SqlParameter("@FBMLD",edtFBMLD.Text.Trim()),	
										new SqlParameter("@FAQK",edtFAQK.Text.Trim()),	
									  new SqlParameter("@FPAQK",txtFPAQK.Text.Trim()),	
										new SqlParameter("@FDZP",edtFDZP.Text.Trim()),	
									  new SqlParameter("@FGT",txtFGT.Text.Trim()),	

										//FDALL,FXFB,FDRS,FBMLD,FAQK,FPAQK,FDZP,FGT
										 ...
										 new SqlParameter("@FState","0"),
										  new SqlParameter("@FDBPD",edtDB.Text.Trim())
                 };

					if (SqlHelper.ExecuteNonQuery(YXK3FZ.DataClass.DataBase.m_Connstr, sSQL, par) > 0)
					{
						//获取当前的保存内码
						sSQL = "select ID FROM TAB07 WHERE FClassID=@FClassID and FBoatID=@FBoatID and  FDate=@FDate ";
				...
				{
					sSQL = " update TAB07 set FRunTimeByHour=@FRunTimeByHour	,FDownTimeByHour=@FDownTimeByHour,FAmountOfFinish=@FAmountOfFinish, ";
					...
					sSQL += " FDALL=@FDALL,FXFB=@FXFB,FDRS=@FDRS,FBMLD=@FBMLD,FAQK=@FAQK,FPAQK=@FPAQK,FDZP=@FDZP,FGT=@FGT,";
					sSQL += " FRemark=@FRemark,FNewDate=@FNewDate,FDBPD=@FDBPD where ID=@ID	 ";
					SqlParameter[] par = new SqlParameter[] 
                 {
			...
			/// <param name="sender"></param>
			/// <param name="e"></param>
			private void edtDB_TextChanged(object sender, EventArgs e)
			{
				//double A = Convert.ToDouble(edtFDALL.Text) - Convert.ToDouble(edtFCar.Text) - Convert.ToDouble(edtFStoreTC.Text) - Convert.ToDouble(edtFStoreYK.Text) - Convert.ToDouble(edtFStorePK.Text) - Convert.ToDouble(edtFCommPD.Text) - Convert.ToDouble(edtFXFB.Text) - Convert.ToDouble(edtFAQK.Text) - Convert.ToDouble(edtFCommSS.Text) - Convert.ToDouble(edtFDZP.Text) - Convert.ToDouble(edtDB.Text);
			...
			/// <param name="sender"></param>
			/// <param name="e"></param>
			private void edtFDZP_TextChanged(object sender, EventArgs e)
			{
				//double A = Convert.ToDouble(edtFDALL.Text) - Convert.ToDouble(edtFCar.Text) - Convert.ToDouble(edtFStoreTC.Text) - Convert.ToDouble(edtFStoreYK.Text) - Convert.ToDouble(edtFStorePK.Text) - Convert.ToDouble(edtFCommPD.Text) - Convert.ToDouble(edtFXFB.Text) - Convert.ToDouble(edtFAQK.Text) - Convert.ToDouble(edtFCommSS.Text) - Convert.ToDouble(edtFDZP.Text) - Convert.ToDouble(edtDB.Text);
