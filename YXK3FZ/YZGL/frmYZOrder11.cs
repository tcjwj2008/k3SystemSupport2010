...
{
    public partial class frmYZOrder11 : Form
    {
			...
			public frmYZOrder11(string sMasterState, string sID)
			{
				InitializeComponent();
			...
			private void LoadDate(string sID)
			{
				...
				if (dttSQL.Rows.Count > 0)
				{
					...
					cobFOrderClass.SelectedValue = dttSQL.Rows[0]["FOrderClass"].ToString();
					...
					txtFOilStoreByTon.Text = dttSQL.Rows[0]["FOilStoreByTon"].ToString();
					edtFMOilStoreByTon.Text = dttSQL.Rows[0]["FMOilStoreByTon"].ToString();
					edtFNum1.Text = dttSQL.Rows[0]["FNum1"].ToString();
					edtFNum2.Text = dttSQL.Rows[0]["FNum2"].ToString();
			...
			private void InitCobDate()
			{
				...
				sSQL = " SELECT 1 FOrderID,  '否'  FOrderClass UNION SELECT 2 FOrderID,  '是'  FOrderClass   ORDER BY FOrderID";
				DataTable dttSQL03 = k3db.GetDataTable(sSQL, "03");
				cobFOrderClass.DataSource = dttSQL03;
				cobFOrderClass.ValueMember = "FOrderClass";
				cobFOrderClass.DisplayMember = "FOrderClass";
				cobFOrderClass.SelectedIndex = -1;
			...
			private void btnSave_Click(object sender, EventArgs e)
			{
				...
				if (cobFOrderClass.SelectedIndex == -1)
				{
					MessageBox.Show("是否当天最后一班不能为空");
					cobFOrderClass.Focus();
					return;
				}
				...
				{
					MessageBox.Show("编织袋-耗量(万条)不能为空");
					edtFConsumptionByTon.Focus();
					return;
				}	

				if (edtFMOilStoreByTon.Text == string.Empty)
				{
					MessageBox.Show("粕出库量(吨)不能为空");
					edtFMOilStoreByTon.Focus();
					return;
				}

				if (edtFNum1.Text == string.Empty)
				{
					MessageBox.Show("粕库存量-1#(吨)不能为空");
					edtFNum1.Focus();
					return;
				}
				if (edtFNum2.Text == string.Empty)
				{
					MessageBox.Show("粕库存量-2#(吨)不能为空");
					edtFNum2.Focus();
					return;
				}
				...
				double E26 = Convert.ToDouble(edtFAmountOfFinish.Text);
				double E27 = Convert.ToDouble(edtFConsumptionByTon.Text);

				if (E26 == 0)
				{
					txtFOilStoreByTon.Text = "0";
				}
				else
				{
					txtFOilStoreByTon.Text = Math.Round(E27 / E26, 2).ToString();
				}
				...
				sSQL = " select * from TAB11 where FClassID='" + cobFClassID.SelectedValue + "' ";
				sSQL += " and FBoatID='" + cobFBoatID.SelectedValue + "'";
				sSQL += " and FDate='" + txtFDate.Text + "'";
				sSQL += " and FOrderClass='" + cobFOrderClass.Text + "'";
				...
				if (dttSQL.Rows.Count > 0)
				{
					MessageBox.Show("日期为" + txtFDate.Text + ",班次为" + cobFClassID.Text + ",船次" + cobFBoatID.Text + ",排班顺序为" + cobFOrderClass.Text + ",数据已保存");
					return;
				}
				...
				{
					sSQL = " INSERT INTO TAB11 ";
					sSQL += " ( ";
					sSQL += " FClassID, FBoatID ,FOrderClass,FDate,FRunTimeByHour	,FDownTimeByHour,FAmountOfFinish	,FConsumptionByTon	,FOilStoreByTon,FMOilStoreByTon,FNum1,FNum2, ";
					...
					SqlParameter[] par = new SqlParameter[] 
                 {
                     ...
                     new SqlParameter("@FBoatID",cobFBoatID.SelectedValue),
										 new SqlParameter("@FOrderClass",cobFOrderClass.SelectedValue),
										 ...
										 new SqlParameter("@FOilStoreByTon",txtFOilStoreByTon.Text.Trim()),
										 new SqlParameter("@FMOilStoreByTon",edtFMOilStoreByTon.Text.Trim()),

										 new SqlParameter("@FNum1",edtFNum1.Text.Trim()),
										 new SqlParameter("@FNum2",edtFNum2.Text.Trim()),
					...
					{
						//获取当前的保存内码
						sSQL = "select ID FROM TAB11 WHERE FClassID=@FClassID and FBoatID=@FBoatID and  FDate=@FDate ";
				...
				{
					sSQL = " update TAB11 set FRunTimeByHour=@FRunTimeByHour	,FDownTimeByHour=@FDownTimeByHour,FAmountOfFinish=@FAmountOfFinish	,FConsumptionByTon=@FConsumptionByTon	,FOilStoreByTon=@FOilStoreByTon,FMOilStoreByTon=@FMOilStoreByTon,FNum1=@FNum1,FNum2=@FNum2, ";
