...
{
	public partial class frmYZOrderList08 : Form
	{
		DataBase k3db = null;

		public frmYZOrderList08()
		{
			InitializeComponent();
		...
		private void frmYZOrderList08_Load(object sender, EventArgs e)
		{
			LoadPower();//加载权限
		...
		private bool getBillState(string sType)
		{
			if (bdsMaster.Current != null)
			{
				...
				sSQL += " FROM TAB08 where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
				DataTable dttSQL = k3db.GetDataTable(sSQL, "A");
		...
		private void btnSel_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				frmYZOrder08 frmYZOrder08 = new frmYZOrder08("SEL", ((DataRowView)bdsMaster.Current).Row["ID"].ToString());
				frmYZOrder08.Text = frmYZOrder08.Text;
				frmYZOrder08.Show();
		...
		{
			frmYZOrder08 frmYZOrder08 = new frmYZOrder08("ADD", "");
			frmYZOrder08.Text = frmYZOrder08.Text;
			frmYZOrder08.Show();
		}
		//修改
		private void tsbtnEdit_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				if (getBillState("修改"))
				{
					frmYZOrder08 frmYZOrder08 = new frmYZOrder08("EDIT", ((DataRowView)bdsMaster.Current).Row["ID"].ToString());
					frmYZOrder08.Text = frmYZOrder08.Text;
					frmYZOrder08.Show();
				}
		...
		private void btnDelete_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				...
				if (MessageBox.Show("确定要删除当前单据吗？", "系统提示", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
				{
					...
					{
						string sSQL = string.Empty;
						sSQL = " delete from  TAB08 where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
		...
		private void btnCheck_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				...
				{
					string sSQL = string.Empty;
					sSQL = " update TAB08 set FState=1,FCheckUser='" + PropertyClass.OperatorName + "',FCheckDate='" + k3db.GetCurrentDate() + "' where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
					if (k3db.ExecDataBySql(sSQL) > 0)
					{
		...
		private void btnHcheck_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				...
				{
					string sSQL = string.Empty;
					sSQL = " update TAB08 set FState=0,FCheckUser=NULL,FCheckDate=NULL where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
					if (k3db.ExecDataBySql(sSQL) > 0)
					{
