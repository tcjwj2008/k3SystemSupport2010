...
{
	public partial class frmYZOrderList01 : Form
	{
		public frmYZOrderList01()
		{
			InitializeComponent();
		...
		private void frmYZOrderList01_Load(object sender, EventArgs e)
		{			
			LoadPower();//加载权限
		...
		private void getDate(string sSQLWH) //加载数据
		{
			...
			bdsMaster.DataSource = dttSQL;
			 ...
			 sSQL += " cast(SUM(FConsumptionByTon) as decimal(18,4)) FConsumptionByTon,  ";
		...
		private bool getBillState(string sType)
		{
			if (bdsMaster.Current != null)
			{
				...
				sSQL += " FROM TAB01 where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
				DataTable dttSQL = k3db.GetDataTable(sSQL, "A");
		...
		private void btnSel_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				frmYZOrder01 frmYZOrder01 = new frmYZOrder01("SEL", ((DataRowView)bdsMaster.Current).Row["ID"].ToString());
				frmYZOrder01.Text = frmYZOrder01.Text ;
				frmYZOrder01.Show();
		...
		{
			frmYZOrder01 frmYZOrder01 = new frmYZOrder01("ADD","");
			frmYZOrder01.Text = frmYZOrder01.Text ;
			frmYZOrder01.Show();
		}
		//修改
		private void tsbtnEdit_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				if (getBillState("修改"))
				{
					frmYZOrder01 frmYZOrder01 = new frmYZOrder01("EDIT", ((DataRowView)bdsMaster.Current).Row["ID"].ToString());
					frmYZOrder01.Text = frmYZOrder01.Text;
					frmYZOrder01.Show();
				}
		...
		private void btnDelete_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				...
				if (MessageBox.Show("确定要删除当前单据吗？", "系统提示", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
				{
					...
					{
						string sSQL = string.Empty;
						sSQL = " delete from  TAB01 where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
		...
		private void btnCheck_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				...
				{
					string sSQL = string.Empty;
					sSQL = " update TAB01 set FState=1,FCheckUser='" + PropertyClass.OperatorName + "',FCheckDate='" + k3db.GetCurrentDate() + "' where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
					if (k3db.ExecDataBySql(sSQL) > 0)
					{
		...
		private void btnHcheck_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				...
				{
					string sSQL = string.Empty;
					sSQL = " update TAB01 set FState=0,FCheckUser=NULL,FCheckDate=NULL where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
					if (k3db.ExecDataBySql(sSQL) > 0)
					{
		...
		private void btnConfirmer_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				...
				{
					string sSQL = string.Empty;
					sSQL = " update TAB01 set FState=2 where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
					if (k3db.ExecDataBySql(sSQL) > 0)
					{
