...
{
	public partial class frmYZOrderList06 : Form
	{
		DataBase k3db = null;

		public frmYZOrderList06()
		{
			InitializeComponent();
		...
		private void frmYZOrderList06_Load(object sender, EventArgs e)
		{
			LoadPower();//加载权限
		...
		private bool getBillState(string sType)
		{
			if (bdsMaster.Current != null)
			{
				...
				sSQL += " FROM TAB06 where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
				DataTable dttSQL = k3db.GetDataTable(sSQL, "A");
		...
		private void btnSel_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				frmYZOrder06 frmYZOrder06 = new frmYZOrder06("SEL", ((DataRowView)bdsMaster.Current).Row["ID"].ToString());
				frmYZOrder06.Text = frmYZOrder06.Text;
				frmYZOrder06.Show();
		...
		{
			frmYZOrder06 frmYZOrder06 = new frmYZOrder06("ADD", "");
			frmYZOrder06.Text = frmYZOrder06.Text;
			frmYZOrder06.Show();
		}
		//修改
		private void tsbtnEdit_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				if (getBillState("修改"))
				{
					frmYZOrder06 frmYZOrder06 = new frmYZOrder06("EDIT", ((DataRowView)bdsMaster.Current).Row["ID"].ToString());
					frmYZOrder06.Text = frmYZOrder06.Text;
					frmYZOrder06.Show();
				}
		...
		private void btnDelete_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				...
				if (MessageBox.Show("确定要删除当前单据吗？", "系统提示", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
				{
					...
					{
						string sSQL = string.Empty;
						sSQL = " delete from  TAB06 where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
		...
		private void btnCheck_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				...
				{
					string sSQL = string.Empty;
					sSQL = " update TAB06 set FState=1,FCheckUser='" + PropertyClass.OperatorName + "',FCheckDate='" + k3db.GetCurrentDate() + "' where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
					if (k3db.ExecDataBySql(sSQL) > 0)
					{
		...
		private void btnHcheck_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				...
				{
					string sSQL = string.Empty;
					sSQL = " update TAB06 set FState=0,FCheckUser=NULL,FCheckDate=NULL where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
					if (k3db.ExecDataBySql(sSQL) > 0)
					{
