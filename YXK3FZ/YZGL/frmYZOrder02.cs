...
{
	public partial class frmYZOrder02 : Form
    {
			...
			/// <param name="sID">内码ID</param>
						public frmYZOrder02(string sMasterState, string sID)
          {
              InitializeComponent();
			...
			private void LoadDate(string sID)
			{
				...
				if (dttSQL.Rows.Count > 0)
				{
					...
					txtFPerConsumption.Text = dttSQL.Rows[0]["FPerConsumption"].ToString();

					edtFAmountOfFinish2.Text = dttSQL.Rows[0]["FAmountOfFinish2"].ToString();
					edtFConsumptionByTon2.Text = dttSQL.Rows[0]["FConsumptionByTon2"].ToString();
					txtFPerConsumption2.Text = dttSQL.Rows[0]["FPerConsumption2"].ToString();
			...
			private void btnSave_Click(object sender, EventArgs e)
			{
				...
				if (edtFAmountOfFinish2.Text == string.Empty)
				{
					MessageBox.Show("蒸汽量(吨)不能为空");
					edtFAmountOfFinish2.Focus();
					return;
				}

				if (edtFConsumptionByTon.Text == string.Empty)
				{
					MessageBox.Show("煤耗-耗量(吨)不能为空");
					edtFConsumptionByTon.Focus();
					return;
				}
				if (edtFConsumptionByTon2.Text == string.Empty)
				{
					MessageBox.Show("蒸汽耗-耗量(吨)不能为空");
					edtFConsumptionByTon2.Focus();
					return;
				}
				...
				sSQL = " select * from TAB02 where FClassID='" + cobFClassID.SelectedValue + "' ";
				...
				{
					sSQL = " INSERT INTO TAB02 ";
					sSQL += " ( ";
					sSQL += " FClassID, FBoatID ,FDate,FRunTimeByHour	,FDownTimeByHour,FAmountOfFinish	,FConsumptionByTon	,FAmountOfFinish2	,FConsumptionByTon2	, ";
					...
					SqlParameter[] par = new SqlParameter[] 
                 {
                     ...
                     new SqlParameter("@FBoatID",cobFBoatID.SelectedValue),
										 ...
										 new SqlParameter("@FConsumptionByTon",edtFConsumptionByTon.Text.Trim()),
										  ...
										  new SqlParameter("@FAmountOfFinish2",edtFAmountOfFinish2.Text.Trim()),
										 new SqlParameter("@FConsumptionByTon2",edtFConsumptionByTon2.Text.Trim()),
					...
					{
						//获取当前的保存内码
						sSQL = "select ID FROM TAB02 WHERE FClassID=@FClassID and FBoatID=@FBoatID and  FDate=@FDate ";
				...
				{
					sSQL = " update TAB02 set FRunTimeByHour=@FRunTimeByHour	,FDownTimeByHour=@FDownTimeByHour,FAmountOfFinish=@FAmountOfFinish	,FConsumptionByTon=@FConsumptionByTon	, ";
					sSQL += " FAmountOfFinish2=@FAmountOfFinish2	,FConsumptionByTon2=@FConsumptionByTon2	,";
