...
{
	public partial class frmYZWaterElecData : Form
	{
		...
		private void LoadData()
		{
			...
			sSQL += "   Where 1=1 and P2.FName in('水','电') ";
		...
		{
			string sSQL = string.Empty;
			sSQL += " SELECT D.*,e.FCode,  FName='['+p1.Fname+']-'+'['+p2.Fname+']-'+'['+p3.Fname+']',FBoatName=t.FName,FDepartName=t2.FName  FROM YZ_WaterElecData d  ";
			...
			{
				edtFDate.Text = dttSQL.Rows[0]["FDate"].ToString();
		...
		private void frmYZWaterElecData_Load(object sender, EventArgs e)
		{
		...
		private void btnSave_Click(object sender, EventArgs e)
		{
			...
			if (MasterState == "ADD")
			{
				sSQL = " SELECT count(*) from YZ_WaterElecData where FDate=@FDate and FBoatID=@FBoatID and FDepID=@FDepID and FProjectID=@FProjectID  ";

				SqlParameter[] par = new SqlParameter[] 
			           {
									  new SqlParameter("@FDate",edtFDate.Text.Trim()),
			...
			{
				int FID = Convert.ToInt32(((DataRowView)bdsMaster.Current).Row["FID"]);
				sSQL = " SELECT count(*) from YZ_WaterElecData where FDate=@FDate and  FBoatID=@FBoatID and FDepID=@FDepID and FProjectID=@FProjectID and FID<>@FID    ";
			...
			if (MasterState == "ADD")
			{
				sSQL = " insert into YZ_WaterElecData(FDate,FBoatID,FDepID,FProjectID,FValue,FText) values (@FDate,@FBoatID,@FDepID,@FProjectID,@FValue,@FText) ";
				SqlParameter[] par = new SqlParameter[] 
			          {
									 new SqlParameter("@FDate",edtFDate.Text.Trim()),
			               new SqlParameter("@FBoatID",cobFBoatID.SelectedValue),
			...
			{
				sSQL = " update YZ_WaterElecData set FDate=@FDate, FBoatID=@FBoatID,FDepID=@FDepID,FProjectID=@FProjectID,FValue=@FValue,FText=@FText  where FID=@FID";
				SqlParameter[] par = new SqlParameter[] 
			          {
									 new SqlParameter("@FDate",edtFDate.Text.Trim()),
		...
		private void btnDelete_Click(object sender, EventArgs e)
		{
			...
			if (MessageBox.Show("确定要删除当前数据？", "软件提示", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
			{
				int FID = Convert.ToInt32(((DataRowView)bdsMaster.Current).Row["FID"]);
				string sSQL = string.Empty;
				sSQL = "DELETE from YZ_WaterElecData where  FID=@FID  ";
