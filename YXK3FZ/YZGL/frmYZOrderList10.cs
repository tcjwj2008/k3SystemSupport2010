...
{
	public partial class frmYZOrderList10 : Form
	{

		DataBase k3db = null;

		public frmYZOrderList10()
		{
			InitializeComponent();
		...
		private void frmYZOrderList10_Load(object sender, EventArgs e)
		{
			LoadPower();//加载权限
		...
		private void getDate(string sSQLWH) //加载数据
		{
			...
			sSQL += " SELECT Q.*,W.FOilStoreByTon,W.FMOilStoreByTon FROM (";
			...
			sSQL += "  left JOIN (  ";
			sSQL += "  select FDate,FOilStoreByTon,FMOilStoreByTon  FROM TAB10 t WHERE FOrderClass='是'  ";
		...
		private bool getBillState(string sType)
		{
			if (bdsMaster.Current != null)
			{
				...
				sSQL += " FROM TAB10 where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
				DataTable dttSQL = k3db.GetDataTable(sSQL, "A");
		...
		private void btnSel_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				frmYZOrder10 frmYZOrder10 = new frmYZOrder10("SEL", ((DataRowView)bdsMaster.Current).Row["ID"].ToString());
				frmYZOrder10.Text = frmYZOrder10.Text;
				frmYZOrder10.Show();
		...
		{
			frmYZOrder10 frmYZOrder10 = new frmYZOrder10("ADD", "");
			frmYZOrder10.Text = frmYZOrder10.Text;
			frmYZOrder10.Show();
		}
		//修改
		private void tsbtnEdit_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				if (getBillState("修改"))
				{
					frmYZOrder10 frmYZOrder10 = new frmYZOrder10("EDIT", ((DataRowView)bdsMaster.Current).Row["ID"].ToString());
					frmYZOrder10.Text = frmYZOrder10.Text;
					frmYZOrder10.Show();
				}
		...
		private void btnDelete_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				...
				if (MessageBox.Show("确定要删除当前单据吗？", "系统提示", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
				{
					...
					{
						string sSQL = string.Empty;
						sSQL = " delete from  TAB10 where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
		...
		private void btnCheck_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				...
				{
					string sSQL = string.Empty;
					sSQL = " update TAB10 set FState=1,FCheckUser='" + PropertyClass.OperatorName + "',FCheckDate='" + k3db.GetCurrentDate() + "' where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
					if (k3db.ExecDataBySql(sSQL) > 0)
					{
		...
		private void btnHcheck_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				...
				{
					string sSQL = string.Empty;
					sSQL = " update TAB10 set FState=0,FCheckUser=NULL,FCheckDate=NULL where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
					if (k3db.ExecDataBySql(sSQL) > 0)
					{
