...
{
    public partial class frmYZOrder13 : Form
    {
			...
			/// <param name="sID">内码ID</param>
						public frmYZOrder13(string sMasterState, string sID)
          {
              InitializeComponent();
			...
			private void LoadDate(string sID)
			{
				...
				if (dttSQL.Rows.Count > 0)
				{
					...
					edtFSG.Text = dttSQL.Rows[0]["FSG"].ToString();
					edtFYF.Text = dttSQL.Rows[0]["FYF"].ToString();
					edtFSC.Text = dttSQL.Rows[0]["FSC"].ToString();

					edtFCF.Text = dttSQL.Rows[0]["FCF"].ToString();
					edtFJL.Text = dttSQL.Rows[0]["FJL"].ToString();
			...
			private void btnSave_Click(object sender, EventArgs e)
			{
				...
				if (edtFSG.Text == string.Empty)
				{
					MessageBox.Show("安全事故次数不能为空");
					edtFSG.Focus();
					return;
				}
				if (edtFYF.Text == string.Empty)
				{
					MessageBox.Show("安全隐患次数不能为空");
					edtFYF.Focus();
					return;
				}
				if (edtFSC.Text == string.Empty)
				{
					MessageBox.Show("文明生产次数不能为空");
					edtFSC.Focus();
					return;
				}
			
				if (edtFCF.Text == string.Empty)
				{
					MessageBox.Show("处罚次数不能为空");
					edtFCF.Focus();
					return;
				}
				if (edtFJL.Text == string.Empty)
				{
					MessageBox.Show("奖励次数不能为空");
					edtFJL.Focus();
					return;
				}
				...
				sSQL = " select * from TAB13 where FClassID='" + cobFClassID.SelectedValue + "' ";
				...
				{
					sSQL = " INSERT INTO TAB13 ";
					sSQL += " ( ";
					sSQL += " FClassID, FBoatID ,FDate,FRunTimeByHour	,FDownTimeByHour, ";
					sSQL += " FSG, FYF, FSC, FCF, FJL ,";
					...
					SqlParameter[] par = new SqlParameter[] 
                 {
                     ...
                     new SqlParameter("@FBoatID",cobFBoatID.SelectedValue),
										 ...
										 new SqlParameter("@FSG",edtFSG.Text.Trim()),
										 new SqlParameter("@FYF",edtFYF.Text.Trim()),
										  new SqlParameter("@FSC",edtFSC.Text.Trim()),
										 new SqlParameter("@FCF",edtFCF.Text.Trim()),
										  new SqlParameter("@FJL",edtFJL.Text.Trim()),
					...
					{
						//获取当前的保存内码
						sSQL = "select ID FROM TAB13 WHERE FClassID=@FClassID and FBoatID=@FBoatID and  FDate=@FDate ";
				...
				{
					sSQL = " update TAB13 set FRunTimeByHour=@FRunTimeByHour	,FDownTimeByHour=@FDownTimeByHour, ";
					sSQL += " FSG=@FSG, FYF=@FYF, FSC=@FSC, FCF=@FCF, FJL=@FJL ";
