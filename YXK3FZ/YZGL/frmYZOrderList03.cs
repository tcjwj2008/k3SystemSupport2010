...
{
	public partial class frmYZOrderList03 : Form
	{
		DataBase k3db = null;

		public frmYZOrderList03()
		{
			InitializeComponent();
		...
		private void frmYZOrderList03_Load(object sender, EventArgs e)
		{
			LoadPower();//加载权限
		...
		private void getDate(string sSQLWH) //加载数据
		{
			...
			sSQL += " cast( (SUM(FAmountOfFinish)/count(*)) as decimal(18,2)) FAmountOfFinish,  ";
			sSQL += " cast( (SUM(FConsumptionByTon)/count(*)) as decimal(18,2)) FConsumptionByTon,  ";
		...
		private bool getBillState(string sType)
		{
			if (bdsMaster.Current != null)
			{
				...
				sSQL += " FROM TAB03 where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
				DataTable dttSQL = k3db.GetDataTable(sSQL, "A");
		...
		private void btnSel_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				frmYZOrder03 frmYZOrder03 = new frmYZOrder03("SEL", ((DataRowView)bdsMaster.Current).Row["ID"].ToString());
				frmYZOrder03.Text = frmYZOrder03.Text;
				frmYZOrder03.Show();
		...
		{
			frmYZOrder03 frmYZOrder03 = new frmYZOrder03("ADD", "");
			frmYZOrder03.Text = frmYZOrder03.Text;
			frmYZOrder03.Show();
		}
		//修改
		private void tsbtnEdit_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				if (getBillState("修改"))
				{
					frmYZOrder03 frmYZOrder03 = new frmYZOrder03("EDIT", ((DataRowView)bdsMaster.Current).Row["ID"].ToString());
					frmYZOrder03.Text = frmYZOrder03.Text;
					frmYZOrder03.Show();
				}
		...
		private void btnDelete_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				...
				if (MessageBox.Show("确定要删除当前单据吗？", "系统提示", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
				{
					...
					{
						string sSQL = string.Empty;
						sSQL = " delete from  TAB03 where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
		...
		private void btnCheck_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				...
				{
					string sSQL = string.Empty;
					sSQL = " update TAB03 set FState=1,FCheckUser='" + PropertyClass.OperatorName + "',FCheckDate='" + k3db.GetCurrentDate() + "' where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
					if (k3db.ExecDataBySql(sSQL) > 0)
					{
		...
		private void btnHcheck_Click(object sender, EventArgs e)
		{
			if (bdsMaster.Current != null)
			{
				...
				{
					string sSQL = string.Empty;
					sSQL = " update TAB03 set FState=0,FCheckUser=NULL,FCheckDate=NULL where ID='" + ((DataRowView)bdsMaster.Current).Row["ID"] + "'";
					if (k3db.ExecDataBySql(sSQL) > 0)
					{
