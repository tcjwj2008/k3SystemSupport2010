...
{
    public partial class frmYZOrder10 : Form
    {
			...
			/// <param name="sID">内码ID</param>
						public frmYZOrder10(string sMasterState, string sID)
          {
              InitializeComponent();
			...
			private void LoadDate(string sID)
			{
				...
				if (dttSQL.Rows.Count > 0)
				{
					...
					edtFOilStoreByTon.Text = dttSQL.Rows[0]["FOilStoreByTon"].ToString();
			...
			private void btnSave_Click(object sender, EventArgs e)
			{
				...
				{
					MessageBox.Show("四级油出库量(吨)不能为空");
					edtFConsumptionByTon.Focus();
					return;
				}

				if (edtFOilStoreByTon.Text == string.Empty)
				{
					MessageBox.Show("四级油库存量(吨)不能为空");
					edtFOilStoreByTon.Focus();
					return;
				}
				...
				sSQL = " select * from TAB10 where FClassID='" + cobFClassID.SelectedValue + "' ";
				...
				{
					sSQL = " INSERT INTO TAB10 ";
					sSQL += " ( ";
					sSQL += " FClassID, FBoatID ,FOrderClass,FDate,FRunTimeByHour	,FDownTimeByHour,FAmountOfFinish	,FConsumptionByTon	,FOilStoreByTon,FMOilStoreByTon, ";
					...
					SqlParameter[] par = new SqlParameter[] 
                 {
                     ...
                     new SqlParameter("@FBoatID",cobFBoatID.SelectedValue),
										 ...
										 new SqlParameter("@FOilStoreByTon",edtFOilStoreByTon.Text.Trim()),
					...
					{
						//获取当前的保存内码
						sSQL = "select ID FROM TAB10 WHERE FClassID=@FClassID and FBoatID=@FBoatID and  FDate=@FDate ";
				...
				{
					sSQL = " update TAB10 set FRunTimeByHour=@FRunTimeByHour	,FDownTimeByHour=@FDownTimeByHour,FAmountOfFinish=@FAmountOfFinish	,FConsumptionByTon=@FConsumptionByTon	,FOilStoreByTon=@FOilStoreByTon,FMOilStoreByTon=@FMOilStoreByTon, ";
