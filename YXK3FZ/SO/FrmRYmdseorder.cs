...
namespace YXK3FZ.SO
{
    public partial class FrmRYmdseorder : Form
    {
        ...
        string conmd  ;
        public FrmRYmdseorder()
        {
            InitializeComponent();
            ds = dbcon.GetDataSet("SELECT Fdbstr    FROM dbo.YXZTLIST WHERE ID=6", "con");
            DataRow dr = ds.Tables[0].Rows[0];
            conmd = dr["Fdbstr"].ToString();
            db = new DataBase(conmd);
        }
        private void FrmRYmdseorder_Load(object sender, EventArgs e)
        {
            commUse.CortrolButtonEnabled(btnSelect, this);
            commUse.CortrolButtonEnabled(btnOutput, this);
            commUse.CortrolButtonEnabled(btnBselect, this);
        ...
        private void btnSelect_Click(object sender, EventArgs e)
        {
            ...
            try{
                string strfilter = " SELECT * FROM v_rymdseorder222 WHERE 开票时间 BETWEEN  '" + this.dateTimePicker1.Value.ToShortDateString() +
                ...
                if (this.checkBox1.Checked==true)
                    strfilter = strfilter + "  AND  是否取消交易='是'";
                if (this.checkBox2.Checked == true)
                    strfilter = strfilter + "  AND  是否取消交易='否'";
