...
namespace YXK3FZ.CC
{
    public partial class frm_dzp_receipt_other : Form
    {
        public frm_dzp_receipt_other()
        {
            InitializeComponent();
        ...
        private void btnInput_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                db.ExecDataBySql("sp_K3_insertreceipt_other");
                //SetFormCaption;
        ...
        private bool validationData()
        {
            ...
            strSqls.Add(" IF OBJECT_ID('t_dzpreceipt_other') IS NULL CREATE TABLE t_dzpreceipt_other(FNumber VARCHAR(255),FDate DATETIME,FFincDate DATETIME,FClassTypeID VARCHAR(50),FExplanation varchar(255),FAmount DECIMAL(20,8),FPreparer VARCHAR(50),FDepartment VARCHAR(50),FRNumber VARCHAR(255))");
            DataRow dr = null;
            for (int i = 0; i < dttResult.Rows.Count; i++)
            {
                dr = dttResult.Rows[i];
                strSqls.Add("INSERT INTO dbo.t_dzpreceipt_other( FNumber ,FDate ,FFincDate ,FClassTypeID ,FExplanation ,FAmount ,FPreparer ,FDepartment)VALUES  ( '" + dr["单号"].ToString().Trim() + "' , '" + dr["单据日期"].ToString().Trim() + "' , '" + dr["财务日期"].ToString().Trim() + "' ,'" + dr["核算项目编号"].ToString().Trim() + "' ,'" + dr["摘要"].ToString().Trim() + "' ,'" + dr["结算实收金额"].ToString().Trim() + "' , '" + dr["制单人"].ToString().Trim() + "' ,'" + dr["部门"].ToString().Trim() + "'  )");
            }
        ...
        private void toolStripButton1_Click(object sender, EventArgs e)
        {
            ...
            if (status)
            {
                ...
                //相对共享文件夹的路径
                string fielpath = @"\导入格式\其他收款单.xlsx";
                //获取保存文件的路径
                string filename = theFolder.ToString() + fielpath;
                //执行方法
                Transport(filename, @"D:\", "其他收款单.xlsx");
        ...
        private void frm_dzp_receipt_other_Load(object sender, EventArgs e)
        {
