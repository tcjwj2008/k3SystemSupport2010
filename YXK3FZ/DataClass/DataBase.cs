     ...
     // 执行参数命令文本(无数据库中数据返回)
        ...
        {//执行无参数的存储过程
			...
			//获取服务器当前时间
                ...
                //4.返回DataTable
                public static DataTable ExecuteDataTable(string connStr, string sql, CommandType cmdType, params SqlParameter[] pms)
                {
                    DataTable dt = new DataTable();
                ...
                public static int ExecuteNonQuery(string connStr,string sql, CommandType cmdType, params SqlParameter[] pms)
                {
