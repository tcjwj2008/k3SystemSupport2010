...
{
    public partial class frm_yunrui_Item : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ds = k3db.GetProcDataSet("sp_check_Item_csp", inputParameters);
                if (ds.Tables[0].Rows.Count == 0)
                {
                    MessageBox.Show("当前旧物料代码不存在，无法保存修改");
                    return;
                }
                ...
                ds = k3db.GetProcDataSet("sp_Save_Item_csp", inputParameters);
                if (ds.Tables[0].Rows.Count == 0)
                {
