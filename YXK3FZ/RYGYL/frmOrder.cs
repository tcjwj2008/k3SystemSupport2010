...
namespace YXK3FZ.RYGYL
{
	public partial class frmOrder : Form
	{
		...
		//构造函数
		public frmOrder(string zttype, string typestr, string billid)
		{
		...
		public void selLoad(string Finterid)
		{
			...
			{
				//cobBatch.SelectedItem = selds.Tables[0].Rows[0]["BatchID"];
				//cobMarket.SelectedItem = selds.Tables[0].Rows[0]["MarketID"];

				//cobBatch.Text = selDB.GetSingleObject(" SELECT isnull(FName,'') FName FROM SP_BaseEntry WHERE FID='" + selds.Tables[0].Rows[0]["BatchID"] + "'").ToString();
				//cobMarket.Text = selDB.GetSingleObject("SELECT isnull(FName,'') FName FROM SP_BaseEntry WHERE FID='" + selds.Tables[0].Rows[0]["MarketID"] + "'").ToString();
