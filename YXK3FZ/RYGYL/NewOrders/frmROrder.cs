...
namespace YXK3FZ.RYGYL
{
	public partial class frmROrder : Form
	{
		...
		//构造函数
		public frmROrder(string zttype, string typestr, string billid) //构造函数
		{
			...
			if (this.ZtType == "R") //肉业
			{
				...
				this.dgvDetail.Columns["Column_Fzlbz"].Visible = false;   //质量标准
				this.dgvDetail.Columns["Column_Fpsyq"].Visible = false;   //配送要求
		...
		private void frmOrder_Load(object sender, EventArgs e) //窗体加载
		{
			//权限控制
			...
			{  
				...
				this.txtFbillno.Text = commUse.SelBillno("yx_order", ZtRyconstring);//单据编号			
				this.txtFBillerName.Text = PropertyClass.OperatorName; //制单人:
				this.DT_pdate.Value = DateTime.Now.AddDays(1); //配送日期
				this.txtFdate.Text = DateTime.Now.ToString("yyyy-MM-dd"); //制单日期
				this.txtSumFamout.Text = "0"; //销售金额合计:
		...
		public void selLoad(string Finterid) //加载数据
		{
			this.Text = "销售订单--查询";
			...
			this.FcurID = selds.Tables[0].Rows[0]["FcurID"].ToString(); //客户内码
			this.FdepID = selds.Tables[0].Rows[0]["FdepID"].ToString(); //部门内码

			this.txtFbillno.Text = this.Fbillno;
			this.txtFcurname.Text = selds.Tables[0].Rows[0]["fcurname"].ToString();      //客户名称
			this.txtFcurNumber.Text = selds.Tables[0].Rows[0]["fcurnumber"].ToString(); //客户代码
			this.txtFcurAdder.Text = selds.Tables[0].Rows[0]["FAddress"].ToString();   //客户地址
			this.txtFdepName.Text = selds.Tables[0].Rows[0]["fdepname"].ToString();    //部门名称
			this.txtFdepNumber.Text = selds.Tables[0].Rows[0]["fdepnumber"].ToString(); //部门代码
			this.txtFempName.Text = selds.Tables[0].Rows[0]["FempName"].ToString(); //业务员
			this.DT_pdate.Value = DateTime.Parse(selds.Tables[0].Rows[0]["Fpdate"].ToString());  //配送日期
		
			this.txtFnote.Text = selds.Tables[0].Rows[0]["Fnote"].ToString(); //备注

			this.txtSumFamout.Text = selds.Tables[0].Rows[0]["FSumFamout"].ToString(); //销售金额合计:		
			this.txtFBillerName.Text = selds.Tables[0].Rows[0]["FBillerName"].ToString();//制单人
			this.txtFConfirmerName.Text = selds.Tables[0].Rows[0]["FConfirmerName"].ToString(); //确认人
			this.txtFcheckName.Text = selds.Tables[0].Rows[0]["FcheckName"].ToString(); //审核人

			try
			{
				this.txtFdate.Text = DateTime.Parse(selds.Tables[0].Rows[0]["FBillerdate"].ToString()).ToString("yyyy-MM-dd");//制单日期
			}
			catch
			{
				this.txtFdate.Text = "";
			...
			try
			{
				this.txtFConfirmerDate.Text = DateTime.Parse(selds.Tables[0].Rows[0]["FConfirmerdate"].ToString()).ToString("yyyy-MM-dd"); //确认日期
			}
			catch
			{
			...
			try
			{
				this.txtFcheckdate.Text = DateTime.Parse(selds.Tables[0].Rows[0]["Fcheckdate"].ToString()).ToString("yyyy-MM-dd"); //审核日期
			}
			catch
			{ 
			...
			this.txtFStause.Text = selds.Tables[0].Rows[0]["status"].ToString(); //状态
			...
			sqlstr += " t.FModel Column_Fmodel,u.Funit Column_FunitName,u.Fqty Column_Fqty,u.Fzlbz Column_Fzlbz,u.Fpsyq Column_Fpsyq,";
			sqlstr += " u.Fnote Column_Fnote,u.FConversion Column_FConversion,u.Fk3unit Column_K3unitname,u.Fk3qty Column_K3Qty,";
			sqlstr += " '' Column_JYqty,u.Fprice Column_Fprice,u.Famout Column_Famout,u.Fpighead Column_Fpighead,";
			sqlstr += " ISNULL(Fbigpig,0) Column_Fbigpig,ISNULL(Fmediumpig,0) Column_Fmediumpig ,ISNULL(Fsmallpig,0) Column_Fsmallpig,FGYYQ Column_FGYYQ,ISNULL(Fmarrypig,0) Column_Fmarrypig";
			sqlstr += " FROM yx_orderentry  u ";
			sqlstr += " INNER JOIN t_icitem t ON u.Fitemid=t.FItemID ";
			sqlstr += " WHERE finterid='" + Finterid + "'";	

			selds = selDB.GetDataSet(sqlstr, "sel"); //得到查询结果
		...
		private void textBox13_TextChanged(object sender, EventArgs e) //录入客户名称或代码显示过滤界面
		{
			SELtype = 0;
		...
		private void dataGridView2_CellDoubleClick(object sender, DataGridViewCellEventArgs e) //选中后(客户SELtype=0,产品SELtype=1)
		{
		...
		private void dgvDetail_CellStateChanged(object sender, DataGridViewCellStateChangedEventArgs e) //计算行的数量 金额
		{
			rowJiSuan();
		}

		private void dgvDetail_CellValueChanged(object sender, DataGridViewCellEventArgs e) //单元格的值变化时
		{
			if (e.RowIndex > -1 && e.ColumnIndex > -1)
			{
		...
		private void dgvDetail_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e) //单元格变化时
		{
			int row;
		...
		void tb_KeyPress(object sender, KeyPressEventArgs e) //文本变化时  在单元格变化时调用
		{
			SELtype = 1;
			int row;
			row = this.dgvDetail.CurrentCell.RowIndex;//行
		...
		public int GetisICC(string FcurID, string FdepID) //判断是否启用信用 
		{  //判断是否启用信用 
			int isICC = 0;
		...
		public int GetPriceFrom(string FcurID, string FdepID) //判断单价取值方向
		{  
			int PFrom = 0;
			DataBase dbIcc = new DataBase(ZtRyconstring);
			SqlParameter param1 = new SqlParameter("@FcurID", SqlDbType.VarChar);
		...
		public int GetIsFunitTou(string FcurID, string FdepID)  //判断单价取值方向
		{ 
			int IsFunitTou = 0;
			DataBase dbIcc = new DataBase(ZtRyconstring);
			SqlParameter param1 = new SqlParameter("@FcurID", SqlDbType.VarChar);
		...
		public int GetisUPrice(string FcurID, string FdepID) //判断单价取值方向
		{  
			int IsFunitTou = 0;
			DataBase dbIcc = new DataBase(ZtRyconstring);
			SqlParameter param1 = new SqlParameter("@FcurID", SqlDbType.VarChar);
		...
		public string getItemUnitFConversion(string fitemid, string unitname, string k3unitname) //取得换算率
		{
		...
		private void btnSave_Click(object sender, EventArgs e) //保存事件
		{
			...
			PSql = "INSERT INTO " + ordertablename + " ( Finterid ,Fbillno , FcurID ,FempName ,Fpdate , FdepID ," +
