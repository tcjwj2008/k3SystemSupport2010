...
{
	public partial class frmSOrder : Form
	{
		...
		//构造函数
		public frmSOrder(string zttype, string typestr, string billid)
		{
			...
			if (this.ZtType == "S") //食品
			{
				...
				cobMarket.DataSource = dttMarket;
				cobMarket.ValueMember = "FID";
				cobMarket.DisplayMember = "FName";
				cobMarket.SelectedIndex = -1;

				sSQL = "SELECT FID,FName FROM SP_BaseEntry WHERE FUID=2"; //批次
				DataTable dttBatch = DB.GetDataSet(sSQL, "sel").Tables[0];
				cobBatch.DataSource = dttBatch;
				cobBatch.ValueMember = "FID";
				cobBatch.DisplayMember = "FName";
		...
		public void selLoad(string Finterid)
		{
			 ...
			 if (this.ZtType == "S") //食品
			{
				...
				sqlstr += " BatchIDDesc=e2.FName,MarketIDDesc=e1.FName, ";
				...
				sqlstr += " LEFT JOIN dbo.SP_BaseEntry e2 ON a.BatchID=e2.FID AND e2.FUID=2 ";
				sqlstr += " LEFT JOIN dbo.SP_BaseEntry e1 ON a.MarketID=e1.FID AND e1.FUID=1 ";
			...
			if (this.ZtType == "S") //如果是食品
			{
				cobBatch.Text = selds.Tables[0].Rows[0]["BatchIDDesc"].ToString();
				cobMarket.Text = selds.Tables[0].Rows[0]["MarketIDDesc"].ToString();

				cobBatch.Enabled = false;
		...
		private void btnSave_Click(object sender, EventArgs e) //保存事件
		{
			...
			if (this.ZtType == "S") //如果是食品
			{
				PSql = "INSERT INTO " + ordertablename + " ( Finterid ,Fbillno , FcurID ,FempName ,Fpdate , Fptime ,Fproperty ,FdepID ,Fcarnum ," +
										 " Fnote ,FSumFamout,FBillerName ,FBillerdate,Fstatus,BatchID,MarketID ) VALUES  (" +
										 ...
										 this.txtFnote.Text + "'," + this.txtSumFamout.Text + ",'" + PropertyClass.OperatorName + "','" + DateTime.Now.ToString("yyyy-MM-dd") + "',0,'" + cobBatch.SelectedValue + "','" + cobMarket.SelectedValue + "')";
		...
		private void btnADD_Click(object sender, EventArgs e)
		{
			if (fType != "SEL")
			{
				//if (MessageBox.Show("当前单据已经改变，是否保存单据？", "软件提示", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Information) == DialogResult.Yes)
				//{
				//  return;
				//}

				DialogResult dr = MessageBox.Show("当前单据已经改变，是否保存单据？", "软件提示", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Information);
		...
		private void cobMarket_SelectedIndexChanged(object sender, EventArgs e)
		{
			if (this.fType == "ADD") //如果是新增
			{
				if (bLoad)
				{
					string sSQL = string.Empty;
					DataBase DB = new DataBase(this.ZtRyconstring);
					sSQL = " select FRemark FROM SP_BaseEntry WHERE FUID=1 and FID='" + cobMarket.SelectedValue + "'";
					DataTable dttFRemark = DB.GetDataSet(sSQL, "sel").Tables[0];
					if (dttFRemark.Rows.Count > 0)
					{
						cobBatch.Text = dttFRemark.Rows[0]["FRemark"].ToString();
					}
				}
